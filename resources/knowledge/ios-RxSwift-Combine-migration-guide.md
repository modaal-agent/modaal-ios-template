...
When tasked with migrating an iOS app from the previous template, agents **MUST COMPLETELY MIGRATE** from RxSwift to Swift Combine. This means:

1. **Remove ALL RxSwift dependencies** — eliminate every reference to `RxSwift`, `RxCocoa`, `RxRelay`, and any other Rx-related frameworks
2. **Replace uber/RIBs with CombineRIBs** — change the dependency to:
   ```swift
   .package(url: "https://github.com/modaal-agent/CombineRIBs.git", from: "2.1.0")
   ```
3. **Convert all reactive code** — use the patterns and examples in this guide as reference, adapting them as appropriate to the specific codebase context

Agents are free to interpret patterns and choose the best refactoring approach given the context. The examples below serve as guidance, not rigid templates.

**Reference Commit:** See [this migration commit](https://github.com/modaal-agent/modaal-ios-template/commit/7325f2cf3ee372d5e88daff8f254b113e8fd8668) for a complete example of this migration.

---

## Overview

This guide provides patterns for migrating from RxSwift to Swift's native Combine framework. The migration uses the CombineRIBs library which provides Combine-based implementations of Uber's RIBs architecture patterns.

## Pattern Mappings

Use the following mappings as reference when converting RxSwift code to Combine.
...
The following examples demonstrate common conversion patterns. Adapt these to fit the specific codebase context.
...
**Recommended:** Initialize with a sensible default value (`false`) rather than using an optional wrapper. Use `.filter { condition }` to control when values are emitted to subscribers.
...
**MANDATORY:** Always use `AnyActionHandler` instead of raw closures (`(() -> Void)?`) for UI events in RIBs. Raw closures are prone to strong reference cycles when capturing `self`. `AnyActionHandler` from `SharedUtility` automatically captures the owner weakly, preventing memory leaks.
...
**Recommended Solution:** Initialize with a sensible default value and use `.filter` to control emissions:
...
**Problem:** Firebase listeners must be removed when the subscription is cancelled to prevent memory leaks.
...
**Problem:** Firebase Storage tasks should be cancelled when the publisher is cancelled to prevent orphaned operations.
...
**Solution:** Replace with `DispatchQueue` directly:
...
**Problem:** Raw closures (`(() -> Void)?`) used for UI events create strong reference cycles when capturing `self` in Presenters.

**MANDATORY Solution:** Use `AnyActionHandler` from `SharedUtility` instead of raw closures:
...
The following file categories are typically affected during migration. Use this as a reference checklist.
...
Verify the following before marking the migration complete:

- [ ] App builds without errors
- [ ] App launches successfully
- [ ] Splash screen displays and transitions to main
- [ ] Resources loading with timeout fallback works
- [ ] Firebase Firestore document observation works (if applicable)
- [ ] Firebase Storage operations work (if applicable)
- [ ] No memory leaks (verify listener cleanup using Instruments)
- [ ] Error handling propagates correctly
- [ ] All unit tests pass
- [ ] **No RxSwift/RxCocoa/RxRelay imports remain in the codebase**

## References

- **Example Migration Commit:** [modaal-ios-template migration](https://github.com/modaal-agent/modaal-ios-template/commit/7325f2cf3ee372d5e88daff8f254b113e8fd8668) — Study this commit for a complete migration example
- [CombineRIBs](https://github.com/modaal-agent/CombineRIBs) — Combine-based RIBs implementation (REQUIRED dependency)
